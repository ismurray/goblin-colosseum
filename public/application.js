...
var gameOver = function gameOver(text) {
  $.toast({
    text: text, // Text that is to be shown in the toast
    heading: 'GAME OVER', // Optional heading to be shown on the toast
    icon: 'error', // Type of toast icon
    showHideTransition: 'fade', // fade, slide or plain
    allowToastClose: true, // Boolean value true or false
...
var updateGold = function updateGold(tx) {
  return $.ajax({
...
var createPurchase = function createPurchase(data) {
  return $.ajax({
    url: config.apiOrigin + '/purchases/',
    method: 'POST',
    headers: {
...
var showAllGamesTemplate = __webpack_require__(28);
var showHighScoresTemplate = __webpack_require__(45);
// const gameMessageTemplate = require('../templates/game-message.handlebars')
// const gameEngine = require('./engine.js')
var store = __webpack_require__(1);
var toast = __webpack_require__(5);

var getAllGamesSuccess = function getAllGamesSuccess(data) {
  var showAllGamesHtml = showAllGamesTemplate({ games: data.games });
...
var getGameSuccess = function getGameSuccess(data) {
  ...
  var showGameHtml = showAllGamesTemplate({ games: data });
...
var updateMapUI = function updateMapUI(game) {
  ...
  for (var i = 0; i < game.liveGoblins.length; i++) {
    var _ID = '#mark' + game.liveGoblins[i].position[0];
    var _spotID = _ID + game.liveGoblins[i].position[1];
    switch (game.liveGoblins[i].lastMove) {
      case 'up':
        $(_spotID).html('<img src="http://res.cloudinary.com/ismurray/image/upload/v1520858541/goblin-up.png" alt="goblin" height="42" width="42">');
        break;
      case 'down':
        $(_spotID).html('<img src="http://res.cloudinary.com/ismurray/image/upload/v1520858540/goblin-down.png" alt="goblin" height="42" width="42">');
        break;
      case 'left':
        $(_spotID).html('<img src="http://res.cloudinary.com/ismurray/image/upload/v1520858540/goblin-left.png" alt="goblin" height="42" width="42">');
        break;
      case 'right':
        $(_spotID).html('<img src="http://res.cloudinary.com/ismurray/image/upload/v1520858540/goblin-right.png" alt="goblin" height="42" width="42">');
        break;
      case undefined:
        $(_spotID).html('<img src="http://res.cloudinary.com/ismurray/image/upload/v1520858540/goblin-down.png" alt="player" height="42" width="42">');
        break;
    }
  }
};
...
var deleteGameSuccess = function deleteGameSuccess(id) {
  var currentGameId = $('#current-game').text();
...
var getHighScoresSuccess = function getHighScoresSuccess(data) {
  var showHighScoresHtml = showHighScoresTemplate({ scores: data });
  $('#high-scores-content').html(showHighScoresHtml);
  toast.success('Scores retrieved!');
};

var getHighScoresFailure = function getHighScoresFailure(error) {
...
module.exports = {
  ...
  getHighScoresSuccess: getHighScoresSuccess,
  getHighScoresFailure: getHighScoresFailure,
...
var wakeUpHeroku = function wakeUpHeroku() {
  return $.ajax({
...
var updateGame = function updateGame(gamePack) {
  var data = gamePack.data;
  return $.ajax({
...
var getHighScores = function getHighScores(data) {
  return $.ajax({
...
var shopAPI = __webpack_require__(7);
var shopUI = __webpack_require__(48);
var store = __webpack_require__(1);

var onGetGold = function onGetGold(event) {
  event.preventDefault();

  shopAPI.getGold().then(shopUI.getGoldSuccess).catch(shopUI.getGoldFailure);
};

var onUpdateGold = function onUpdateGold(amount) {
  event.preventDefault();
  // const tx = $('#update-gold input').val()
  shopAPI.updateGold(amount).then(shopUI.updateGoldSuccess).catch(shopUI.updateGoldFailure);
};

var onGetPurchases = function onGetPurchases(event) {
  event.preventDefault();
  shopAPI.getPurchases().then(shopUI.getPurchasesSuccess).catch(shopUI.getPurchasesFailure);
};

var onCreatePurchase = function onCreatePurchase(event) {
  event.preventDefault();
  var purchaseItem = $(this).attr('data-id');
  var data = void 0;
  ...
  if (purchaseItem === 'heal' || purchaseItem !== 'heal' && !store.accountPurchases[purchaseItem]) {
    var purchaseItemName = store.accountPurchases.serverItemNames[purchaseItem];
    data = {
      'purchase': {
  ...
  shopAPI.createPurchase(data).then(shopUI.createPurchaseSuccess).catch(shopUI.createPurchaseFailure);
};
...
var shopEvents = __webpack_require__(16);
...
var shopAPI = __webpack_require__(7);
var store = __webpack_require__(1);
var authUI = __webpack_require__(23);
var toast = __webpack_require__(5);
...
var checkSessionCreds = function checkSessionCreds() {
  var user = sessionStorage.getItem('user');
  if (user !== undefined && user !== null) {
    // console.log('sessionCreds user is ', JSON.parse(user))
    store.user = JSON.parse(user).user;
    // console.log('store.user is ', store.user)
    shopAPI.getGold().then(function () {
      return authUI.signInSuccess(JSON.parse(user));
    }).catch(function (res) {
...
var onSignIn = function onSignIn(event) {
  ...
  var data = getFormFields(this);
  // console.log('data is ', data)
  authAPI.signIn(data).then(function (res) {
    // console.log('res is ', res)
    sessionStorage.setItem('user', JSON.stringify(res));
    return res;
  }).then(function (res) {
    var credentials = JSON.parse(sessionStorage.getItem('user'));
    // console.log('session credentials is ', credentials.user.token)
    return res;
  }).then(authUI.signInSuccess).then(function () {
...
module.exports = {
  addHandlers: addHandlers,
  checkSessionCreds: checkSessionCreds
};
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(0)))
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
  ...
  return "\n\n <ul data-id=\""
    ...
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
    + "</li>\n"
    + ((stack1 = helpers.unless.call(alias3,((stack1 = blockParams[0][0]) != null ? stack1.over : stack1),{"name":"unless","hash":{},"fn":container.program(2, data, 0, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "")
    + ((stack1 = helpers["if"].call(alias3,((stack1 = blockParams[0][0]) != null ? stack1.over : stack1),{"name":"if","hash":{},"fn":container.program(4, data, 0, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "")
    + "  <li>Score: "
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.score : stack1), depth0))
    + "</li>\n  <li>Round: "
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.round : stack1), depth0))
    + "</li>\n"
    + ((stack1 = helpers.unless.call(alias3,((stack1 = blockParams[0][0]) != null ? stack1.over : stack1),{"name":"unless","hash":{},"fn":container.program(6, data, 0, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "")
    + "  <ul>\n\n  </ul>\n</ul>\n\n";
},"2":function(container,depth0,helpers,partials,data) {
    return "  <li>This game is still ACTIVE. You should finish this game to earn gold and publish your score!</li>\n";
},"4":function(container,depth0,helpers,partials,data) {
    return "  <li>Game Status: Completed!</li>\n";
},"6":function(container,depth0,helpers,partials,data,blockParams) {
    var stack1, alias1=container.lambda, alias2=container.escapeExpression;

  return "  <button type=\"button\" class=\"load-button\" data-id=\""
  ...
  return "\n <tr data-id=\""
    ...
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
    + "</td>\n"
    + ((stack1 = helpers.unless.call(alias3,((stack1 = blockParams[0][0]) != null ? stack1.over : stack1),{"name":"unless","hash":{},"fn":container.program(9, data, 0, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "")
    + ((stack1 = helpers["if"].call(alias3,((stack1 = blockParams[0][0]) != null ? stack1.over : stack1),{"name":"if","hash":{},"fn":container.program(11, data, 0, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "")
    + "   <td>"
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.score : stack1), depth0))
    + "</td>\n   <td>"
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.round : stack1), depth0))
    + "</td>\n"
    + ((stack1 = helpers.unless.call(alias3,((stack1 = blockParams[0][0]) != null ? stack1.over : stack1),{"name":"unless","hash":{},"fn":container.program(13, data, 0, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "")
    + " </tr>\n\n\n\n";
},"9":function(container,depth0,helpers,partials,data) {
    return "   <td>Unfinished!</td>\n";
},"11":function(container,depth0,helpers,partials,data) {
    return "   <td>Completed!</td>\n";
},"13":function(container,depth0,helpers,partials,data,blockParams) {
    var stack1, alias1=container.lambda, alias2=container.escapeExpression;

  return "   <td><button type=\"button\" class=\"load-button\" data-id=\""
    + alias2(alias1(((stack1 = blockParams[1][0]) != null ? stack1.id : stack1), depth0))
    + "\">Load Game</button></td>\n   <td><button type=\"button\" class=\"delete-button\" data-id=\""
    + alias2(alias1(((stack1 = blockParams[1][0]) != null ? stack1.id : stack1), depth0))
  ...
  return "<!-- "
    ...
    + " -->\n\n<table class=\"table table-striped table-hover table-responsive\">\n<tr class='header_row'>\n  <td>Game ID:</td>\n  <td>Game Status:</td>\n  <td>Score:</td>\n  <td>Round:</td>\n  <td>Load Game</td>\n  <td>Delete Game</td>\n</tr>\n<p>Unfinished games can be resumed by clicking the 'Load Game' button. Completed games cannot be resumed or deleted.</p>\n"
    + ((stack1 = helpers.each.call(alias1,(depth0 != null ? depth0.games : depth0),{"name":"each","hash":{},"fn":container.program(8, data, 1, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "")
    + "</table>\n";
...
var gameUI = __webpack_require__(8);
var gameEngine = __webpack_require__(47);
var getFormFields = __webpack_require__(10);
var store = __webpack_require__(1);
var shopAPI = __webpack_require__(7);
...
var onGetAllGames = function onGetAllGames(event) {
  event.preventDefault();
  gameAPI.getAllGames().then(gameUI.getAllGamesSuccess).catch(gameUI.getAllGamesFailure);
};
...
var onGetGame = function onGetGame(event) {
  ...
  gameAPI.getGame(id).then(gameEngine.loadGame).then(gameUI.getGameSuccess).catch(gameUI.getGameFailure);
};
...
var onCreateNewGame = function onCreateNewGame(event) {
  event.preventDefault();
  gameAPI.createGame().then(gameEngine.createNewGame).then(gameUI.createGameSuccess).catch(gameUI.createGameFailure);
};
...
var onDeleteGame = function onDeleteGame(event) {
  ...
  var getElement = '[data-id=' + id + ']';
  $(getElement).empty();
  gameAPI.deleteGame(id).then(gameUI.deleteGameSuccess).catch(gameUI.deleteGameFailure);
};

var onMakeMove = function onMakeMove(event) {
  event.preventDefault();
  var direction = event.currentTarget.id.split('-')[1];
  var game = gameEngine.movePlayer(direction, 'attack');
  gameUI.updateMapUI(game);
};

var onSweepAttack = function onSweepAttack(event) {
  event.preventDefault();
  if (store.accountPurchases['sweep']) {
    var game = gameEngine.movePlayer('down', 'sweep');
    gameUI.updateMapUI(game);
  } else {
...
var onBlastAttack = function onBlastAttack(event) {
  event.preventDefault();

  if (store.accountPurchases['blast']) {
    var direction = event.currentTarget.id.split('-')[1];
    var game = gameEngine.movePlayer(direction, 'blast');
    gameUI.updateMapUI(game);
  } else {
...
var onHeal = function onHeal(event) {
  event.preventDefault();
  if (store.accountPurchases.healthPotions > 0) {
    // heal player in game engine
    var game = gameEngine.movePlayer('down', 'heal');
    gameUI.updateMapUI(game);
    // remove a heal purchase from local store and update API
    var purchaseID = store.accountPurchases.healPurchaseIDs.shift();
...
var onGetHighScores = function onGetHighScores(event) {
  event.preventDefault();
  gameAPI.getHighScores().then(gameUI.getHighScoresSuccess).catch(gameUI.getHighScoresFailure);
};
...
var updateUI = function updateUI() {
  ...
  var liveGoblins = findLiveGoblins(localGame.goblinState);
  var gameUiData = {
...
var packageGameData = function packageGameData() {
  var dataPack = {
    id: apiGame.id,
    data: {
      game: {
...
var resetMap = function resetMap(rowLength) {
  map = [];
...
var updateMap = function updateMap(player, goblins) {
...
var setNeighborIndices = function setNeighborIndices(combatant) {
...
var validateNeighborIndices = function validateNeighborIndices(combatant) {
...
var moveGoblin = function moveGoblin(goblin, direction) {
  goblin.lastMove = direction;
  var destination = goblin.neighborIndices[direction];
  var currentPos = goblin.position;
  map = updateMap(localGame.playerState, localGame.goblinState);
...
var movePlayer = function movePlayer(direction, ability) {
  // resetAttackAnimations()
  localGame.playerState.lastMove = direction;
  var destination = localGame.playerState.neighborIndices[direction];
  var currentPos = localGame.playerState.position;
  map = updateMap(localGame.playerState, localGame.goblinState);
  if (ability === 'attack') {
...
var sweepAttack = function sweepAttack(player) {
  addGameMessage('player makes a sweeping attack that strikes at all four sides');
  var attackDestinations = [player.neighborIndices['up'], player.neighborIndices['down'], player.neighborIndices['left'], player.neighborIndices['right']];
  for (var i = 0; i < attackDestinations.length; i++) {
...
// Takes player obect as input to enact an attack at all four directions
var blastAttack = function blastAttack(player, direction) {
  addGameMessage('player shoots a crackling beam of energy that hits all enemies in a line');
  var attackDestinations = blastAttackTargets(player, direction);
...
var heal = function heal(player) {
  var formerHP = player.hp[0];
  player.hp[0] = player.hp[1];
  var text = 'player quaffs a healing potion, raising their HP from ' + formerHP + ' to ' + player.hp[0] + '!';
  addGameMessage(text);
};

var blastAttackTargets = function blastAttackTargets(player, direction) {
  var targets = [];
  var virtualCombatant = { position: [] };
...
var findLiveGoblins = function findLiveGoblins(goblins) {
  var liveGoblins = [];
  for (var i = 0; i < goblins.length; i++) {
    if (goblins[i].alive) {
...
var buryDeadGobs = function buryDeadGobs(goblins) {
  for (var i = 0; i < goblins.length; i++) {
    if (!goblins[i].alive) {
      goblins.splice(i, 1);
    }
...
var findGobByPosition = function findGobByPosition(goblins, givenPosition) {
  return goblins.findIndex(function (goblin) {
    return goblin.position[0] === givenPosition[0] && goblin.position[1] === givenPosition[1] && goblin.alive;
  });
};
...
var attack = function attack(attacker, target) {
  target.hp[0] -= attacker.attackDmg;
  var text = attacker.name + ' attacks ' + target.name + ' for ' + attacker.attackDmg + ' damage! Reducing ' + target.name + ' HP to ' + target.hp[0] + '/' + target.hp[1];
  addGameMessage(text);
...
var targetAttack = function targetAttack(attacker, destination) {
  if (attacker.name === 'player') {
    var gobIndex = findGobByPosition(localGame.goblinState, destination);
    if (gobIndex > -1) {
      var target = localGame.goblinState[gobIndex];
...
var deathCheck = function deathCheck(target) {
  if (target.hp[0] < 1) {
    target.alive = false;
    // if target killed was a gob, increase score by 1
    if (target.name !== 'player') {
      localGame.score += 1;
      var text = 'You have slain ' + target.name + '! Your score is now ' + localGame.score;
      addGameMessage(text);
...
var chasePlayer = function chasePlayer(goblin) {
  var xDiff = goblin.position[1] - localGame.playerState.position[1];
  var yDiff = goblin.position[0] - localGame.playerState.position[0];
  // if further from player along x-axis, move along x-axis
  if (Math.abs(xDiff) > Math.abs(yDiff) || Math.abs(xDiff) === Math.abs(yDiff)) {
...
var goblinTurns = function goblinTurns(goblins) {
  for (var i = 0; i < goblins.length; i++) {
...
var createGoblin = function createGoblin(position, hp, attackDmg, alive) {
  var goblin = {};
  goblin.name = 'goblin';
...
var makeRandomNum = function makeRandomNum(upperLimit) {
  return Math.floor(Math.random() * upperLimit);
};

// returns true if player position overlaps with newPosition. Both params are coords
var playerOverlapCheck = function playerOverlapCheck(newPosition, playerPosition) {
...
var goblinOverlapCheck = function goblinOverlapCheck(newPosition, goblins) {
  var goblinAlreadyAtNewPos = findGobByPosition(goblins, newPosition);
...
var randomizeGobPos = function randomizeGobPos(goblin) {
  var newPosition = [makeRandomNum(5), makeRandomNum(5)];
  var playerPosition = localGame.playerState.position;
  // run overlap checks
  var playerOverlaps = playerOverlapCheck(newPosition, playerPosition);
  var goblinOverlaps = goblinOverlapCheck(newPosition, localGame.goblinState);
...
var spawnCheck = function spawnCheck(round) {
  if (round % 10 === 0) {
    var newGobs = [];
    var newGobAmount = round / 10 + 1;
    var availableSpaces = numberOfOpenMapSpaces();
    newGobAmount = newGobAmount > availableSpaces ? availableSpaces : newGobAmount;
    for (var i = 0; i < newGobAmount; i++) {
      newGobs.push(createGoblin([0, 2], [10, 10], 1, true));
    }
    var text = newGobs.length + ' goblins enter the arena!';
    addGameMessage(text);
    for (var _i = 0; _i < newGobs.length; _i++) {
      localGame.playerState.goblinCount++;
      newGobs[_i].position = randomizeGobPos(newGobs[_i]);
      setNeighborIndices(newGobs[_i]);
      newGobs[_i].name = newGobs[_i].name + localGame.playerState.goblinCount;
      localGame.goblinState.push(newGobs[_i]);
    }
  } else if (round === 1) {
    var _newGobs = [createGoblin([0, 2], [10, 10], 1, true)];
    setNeighborIndices(_newGobs[0]);
    _newGobs[0].name = _newGobs[0].name + localGame.playerState.goblinCount;
    localGame.goblinState.push(_newGobs[0]);
  }
};

// returns the number of open spaces on the map
var numberOfOpenMapSpaces = function numberOfOpenMapSpaces() {
  var counter = 0;
...
var addGameMessage = function addGameMessage(message) {
  var gameMessageTemplate = __webpack_require__(50);
  var timeStamp = new Date();
  var gameMessageHtml = gameMessageTemplate({ messages: [{ text: message, time: timeStamp }] });
...
module.exports = {
  ...
  packageGameData: packageGameData,
  loadGame: loadGame,
...
var store = __webpack_require__(1);
var showAllPurchasesTemplate = __webpack_require__(49);
var toast = __webpack_require__(5);

var getGoldSuccess = function getGoldSuccess(data) {
  $('#user-gold').html(data);
};

var getGoldFailure = function getGoldFailure(error) {
  toast.failure('Error retrieving gold balance!');
  console.log(error);
};

var updateGoldSuccess = function updateGoldSuccess(data) {
  toast.success('Gold Deposited Into Your Account!');
  $('#user-gold').html(data);
};

var updateGoldFailure = function updateGoldFailure(error) {
  toast.failure('No Gold Transferred!');
  console.log(error);
};

// iterate through API purchases data and update local store
var storePurchases = function storePurchases(data) {
  // reset local store of account purchases
  var accountPurchases = {
    healthPotions: 0,
    healPurchaseIDs: [],
    sweep: false,
    blast: false,
...
var getPurchasesSuccess = function getPurchasesSuccess(data) {
  toast.success('Purchases retrieved!');
  storePurchases(data);
  var showPurchasesHtml = showAllPurchasesTemplate({ accountPurchases: [store.accountPurchases] });
  $('#all-purchases-content').html(showPurchasesHtml);
};

var getPurchasesFailure = function getPurchasesFailure(error) {
  toast.failure('Error retrieving purchases!');
  console.log(error);
};

var createPurchaseSuccess = function createPurchaseSuccess(data) {
  toast.success('Purchase successful!');
  var showPurchasesHtml = showAllPurchasesTemplate({ purchases: data });
  $('#all-purchases-content').append(showPurchasesHtml);
  $('#user-gold').html(data.purchase.user.gold);
  $('#get-user-purchases').click();
};

var createPurchaseFailure = function createPurchaseFailure(error) {
  toast.failure('Error processing purchase!');
  console.log(error);
};

module.exports = {
  getGoldSuccess: getGoldSuccess,
  getGoldFailure: getGoldFailure,
  updateGoldSuccess: updateGoldSuccess,
  updateGoldFailure: updateGoldFailure,
  getPurchasesSuccess: getPurchasesSuccess,
  getPurchasesFailure: getPurchasesFailure,
  createPurchaseSuccess: createPurchaseSuccess,
  createPurchaseFailure: createPurchaseFailure
};
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(0)))
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
  ...
  return "\n <ul>\n  <li>Health Potions: "
    + container.escapeExpression(container.lambda(((stack1 = blockParams[0][0]) != null ? stack1.healthPotions : stack1), depth0))
    + "</li>\n"
    + ((stack1 = helpers["if"].call(alias1,((stack1 = blockParams[0][0]) != null ? stack1.sweep : stack1),{"name":"if","hash":{},"fn":container.program(2, data, 0, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "")
    + ((stack1 = helpers.unless.call(alias1,((stack1 = blockParams[0][0]) != null ? stack1.sweep : stack1),{"name":"unless","hash":{},"fn":container.program(4, data, 0, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "")
    + ((stack1 = helpers["if"].call(alias1,((stack1 = blockParams[0][0]) != null ? stack1.blast : stack1),{"name":"if","hash":{},"fn":container.program(6, data, 0, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "")
    + ((stack1 = helpers.unless.call(alias1,((stack1 = blockParams[0][0]) != null ? stack1.blast : stack1),{"name":"unless","hash":{},"fn":container.program(8, data, 0, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "")
    + "</ul>\n\n";
},"2":function(container,depth0,helpers,partials,data) {
    return "  <li>Sweeping Strike: Unlocked!</li>\n";
},"4":function(container,depth0,helpers,partials,data) {
    return "  <li>Sweeping Strike: [Not Yet Purchased]</li>\n";
},"6":function(container,depth0,helpers,partials,data) {
    return "  <li>Lightning Blast: Unlocked!</li>\n";
},"8":function(container,depth0,helpers,partials,data) {
    return "  <li>Lightning Blast: [Not Yet Purchased]</li>\n";
},"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data,blockParams) {
    var stack1;

  return ((stack1 = helpers.each.call(depth0 != null ? depth0 : (container.nullContext || {}),(depth0 != null ? depth0.accountPurchases : depth0),{"name":"each","hash":{},"fn":container.program(1, data, 1, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "");
},"useData":true,"useBlockParams":true});
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
  ...
  return ((stack1 = helpers.each.call(depth0 != null ? depth0 : (container.nullContext || {}),(depth0 != null ? depth0.messages : depth0),{"name":"each","hash":{},"fn":container.program(1, data, 1, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "");
},"useData":true,"useBlockParams":true});
